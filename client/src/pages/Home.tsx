...
export default function Home() {
  ...
  const onRetryWorker = async () => {
    ...
    try {
      ...
      if (!result.ok) {
        const detail = result.output.split("\n").filter(Boolean).filter((line) => !line.includes("Owner notification skipped") && !line.includes("built-in credentials are not configured")).slice(-8).join(" ").slice(-900);
  ...
  const handleSignWallet = async () => {
    ...
    try {
      ...
      const auth = await verifyWalletSignature.mutateAsync({ address: wallet.address, message, signature });
      localStorage.setItem("cargo-proof-wallet-token", auth.token);
